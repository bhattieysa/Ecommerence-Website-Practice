...
export function formatMoney(amount, currency, options) {
    const { locale = COMMERCE_CONFIG.locale, fractionDigits = COMMERCE_CONFIG.fractionDigits, currencyDisplay = COMMERCE_CONFIG.currencyDisplay, } = options || {};
 ...
 */
export function calculateDiscount(originalAmount, currentAmount) {
    // Validate inputs
    if (originalAmount <= 0 ||
        currentAmount <= 0 ||
 ...
 */
export function formatMoneyObject(money, options) {
